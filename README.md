...
- **50 大策略胜率排行（Top50）**：内置约 50 个经典技术策略（多参数的均线/EMA/MACD/RSI/布林带/
  唐奇安/动量 ROC/随机指标 KD/均值回归/威廉指标/CCI/趋势过滤），用近 1 年真实数据回测排名；
  右侧点击任一策略，即在 K 线上标出它的历史买卖点
- **盘前/盘后（夜盘）**：分钟级 K 线与实时报价包含扩展时段数据；页面显示当前时段（盘前/盘中/盘后/休市）。
  免费 IEX 源的盘前盘后覆盖较少，完整扩展时段需在 config.json 用 `"feed":"sip"`（Alpaca 付费源）
